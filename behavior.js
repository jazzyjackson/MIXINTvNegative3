...
function findDIVinBubble(event){ //follows the event bubble up the path, 0 index is event.targe, length-1 index is topmost parent element. bubble until a DIV is hit, return the div.
    for(var each in event.path){
        
        if(event.path[each].tagName == 'DIV'){
            return event.path[each];
            console.log(event.path[each])
        }
    }
    console.log(null);
    return null; //if DIV wasn't in the path, then activeElement should be NULL
}
